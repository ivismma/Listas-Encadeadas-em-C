...
int contagem_Lista(Node *topo, int busca){ // Parecida com a função acima, porém esta CONTA quantas vezes o valor buscado aparece na lista.
	if(topo == NULL) return 0; // 1º caso: Lista vazia ou chegou no último elemento.
		...
		return 1 + contagem_Lista(topo->prox, busca); // 3º caso Elemento buscado encontrado, incrementa 1 e segue procurando possíveis próximas aparições do valor.
	else // 4º caso Não encontrou na iteração atual, segue procurando possíveis próximas aparições e NÃO incrementa na contagem.
